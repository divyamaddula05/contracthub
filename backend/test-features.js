...
async function test() {
  try {
    ...
    // Login as client1
    res = await API.post('/auth/login', { email: 'client@test.com', password: 'password123' });
    if (res.status !== 200) throw new Error(`Client1 login failed: ${res.status}`);
    client1Token = res.data.token;
    log('pass', `Client1 logged in`);
    
    // Login as client2
    res = await API.post('/auth/login', { email: 'client2@gmail.com', password: 'password123' });
    ...
    const client1Data = clients.find(c => c.email === 'client@test.com');
    const client2Data = clients.find(c => c.email === 'client2@gmail.com');
    ...
    const client1Contracts = Array.isArray(res.data) ? res.data : (res.data.contracts || []);
    ...
    if (res.status !== 200) throw new Error(`Get contracts failed: ${res.status}`);
    const client2Contracts = Array.isArray(res.data) ? res.data : (res.data.contracts || []);
    ...
    const testFileName = 'test-v1.pdf';
    fs.writeFileSync(testFileName, 'dummy PDF content');
    
    // Upload a version using multipart form data
    const formData = new FormData();
    formData.append('file', fs.createReadStream(testFileName));
    
    res = await API.post(`/contracts/${contractId}/upload`, 
      formData,
      { 
        headers: { 
    ...
    if (res.status !== 201) throw new Error(`Upload version failed: ${res.status} - ${JSON.stringify(res.data)}`);
    ...
    res = await API.get(`/contracts/${contractId}/versions`, { headers: { 'Authorization': `Bearer ${client1Token}` } });
    if (res.status !== 200) throw new Error(`Get versions failed: ${res.status}`);
    ...
    const updatedVersion = allVersions.find(v => v._id === versionId);
    ...
    const testFileName2 = 'test-v2.pdf';
    fs.writeFileSync(testFileName2, 'dummy PDF content v2');
    
    const formData2 = new FormData();
    formData2.append('file', fs.createReadStream(testFileName2));
    ...
    res = await API.get(`/contracts/${contractId}/versions`, { headers: { 'Authorization': `Bearer ${client1Token}` } });
    if (res.status !== 200) throw new Error(`Get versions failed: ${res.status}`);
    const allVersionsCheck = res.data;
    const rejectedVersion = allVersionsCheck.find(v => v._id === versionId2);
