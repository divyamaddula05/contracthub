Try {
  ...
  $adminResp = Invoke-RestMethod -Uri 'http://localhost:5000/api/auth/login' -Method Post -ContentType 'application/json' -Body $adminBody
  ...
  $contract = Invoke-RestMethod -Uri 'http://localhost:5000/api/contracts' -Method Post -ContentType 'application/json' -Body $contractBody -Headers $headers
  ...
  $uploadRaw = & curl.exe -s -H ("Authorization: Bearer {0}" -f $adminToken) -F ("file=@{0}" -f $samplePath) ("http://localhost:5000/api/contracts/{0}/upload" -f $contract._id)
  ...
  $versions = Invoke-RestMethod -Uri ("http://localhost:5000/api/contracts/{0}/versions" -f $contract._id) -Method Get -Headers $headers
  ...
  $clientResp = Invoke-RestMethod -Uri 'http://localhost:5000/api/auth/login' -Method Post -ContentType 'application/json' -Body $clientBody
  ...
  $approveResp = Invoke-RestMethod -Uri ("http://localhost:5000/api/contracts/{0}/versions/{1}/approve" -f $contract._id, $versionId) -Method Put -Headers $clientHeaders
  Write-Host "Approve response: $($approveResp | ConvertTo-Json -Depth 3)"

  Write-Host 'Fetching audit logs (admin)...'
  $logs = Invoke-RestMethod -Uri ("http://localhost:5000/api/contracts/{0}/logs" -f $contract._id) -Method Get -Headers $headers
