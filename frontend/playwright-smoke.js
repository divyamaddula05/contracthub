...
(async () => {
  ...
  try {
    ...
    const loginResp = await (await fetch('https://contracthub-zwiu.onrender.com/api/auth/login', {
      method: 'POST',
    ...
    await page.goto('https://contracthub-zwiu.onrender.com/', { waitUntil: 'networkidle' });
    ...
    const adminLoginResp = await (await fetch('https://contracthub-zwiu.onrender.com/api/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    await adminPage.goto('https://contracthub-zwiu.onrender.com/', { waitUntil: 'networkidle' });
    await adminPage.waitForTimeout(1200);

    const meResp = await adminPage.evaluate(async () => {
      const res = await fetch('https://contracthub-zwiu.onrender.com/api/auth/me', { headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } });
